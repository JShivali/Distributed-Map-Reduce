...
def start_instances(compute, project, zone, instance):
    request = compute.instances().start(project=project, zone=zone, instance=instance)
    response = request.execute()

    print('VM Instance started : ', instance)


def stop_instances(compute, project, zone, instance):
    request = compute.instances().stop(project=project, zone=zone, instance=instance)
    response = request.execute()

    print('VM Instance Stopped : ', instance)
...
def create_instance(compute, project, zone, name, bucket):
    ...
    image_response = compute.images().getFromFamily(
        project='cloud-map-reduce', family='mapredbase').execute()
